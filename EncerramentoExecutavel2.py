...
COMPANY_PRESETS: Dict[str, Dict[str, object]] = {
   ...
   'EGP': {
    ...
    'TLSA - IMOBILIARIO': {
            'dataEnvio': TODAY_STR,
            'cod_usuario_envio': '222',
            'cod_lote': f'TLSA - IMOBILIARIO{TODAY_STR}',
            'carteira': '112',
    },
...
'MOVIDA PASSIVO': {
            'dataEnvio': TODAY_STR,
            'cod_usuario_envio': '222',
            'cod_lote': f'MOVIDA PASSIVO {TODAY_STR}',
            'carteira': '62',
    },
...
class MigracoesApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            top_header,
            text="📤 Importador de Encerramentos",
            style="Header.TLabel"
        ).pack(side="left")
        ...
        ttk.Button(
            actions,
            text='Enviar ao Banco',
            style="Primary.TButton",
            command=self.on_send
        ).pack(side='left', padx=8)

        # NOVO BOTÃO: consulta/exclusão por lote
        ttk.Button(
            actions,
            text='Consulta/Excluir por Lote...',
            style="Ghost.TButton",
            command=self.open_consulta_lote_window
        ).pack(side='right', padx=8)

        # Botão já existente: Admin Encerramento
        ttk.Button(
            actions,
            text='Admin Encerramento...',
            style="Ghost.TButton",
    ...
    def on_clear(self):
        """Limpa todos os campos da tela principal."""
        # Limpa caminho da planilha
        self.ent_path.delete(0, tk.END)

        # Limpa combobox de sheet
        self.cmb_sheet.set('')
        self.cmb_sheet['values'] = []

        # Limpa combobox de empresa
        ...
        # Atualiza status
        self.set_status('🟢 Pronto. Tela limpa.')

    def open_admin_window(self):
        """Janela auxiliar para excluir linhas e exportar por cod_lote."""
        win = tk.Toplevel(self)
        win.title("Administração da tabela encerramento")
        win.geometry("720x460")
        win.configure(bg=self.pal["bg"])

        container = ttk.Frame(win, padding=16, style="Card.TFrame")
        container.pack(fill="both", expand=True, padx=14, pady=14)

        # ========================
        # 1) Exclusão por CNJ
        # ========================
        self.section_title(container, "🧹 Exclusão de registros")

        frame_del = ttk.Frame(container, style="Card.TFrame")
        frame_del.pack(fill="x", pady=(0, 16))

        ttk.Label(frame_del, text="CNJ para excluir:", style="TLabel").grid(
            row=0, column=0, sticky="w", pady=4
        )
        ent_cnj = ttk.Entry(frame_del, width=40)
        ent_cnj.grid(row=0, column=1, padx=8, pady=4, sticky="w")

        def on_delete_cnj():
            cnj = ent_cnj.get().strip()
            if not cnj:
                messagebox.showwarning("Atenção", "Informe o CNJ que deseja excluir.")
                return

            if not messagebox.askyesno(
                    "Confirmar exclusão",
                    f"Tem certeza que deseja excluir TODAS as linhas com CNJ:\n\n{cnj} ?"
            ):
                return
            ...
            try:
                cur.execute("DELETE FROM encerramento WHERE cnj = %s", (cnj,))
                deleted = cur.rowcount or 0
                conn.commit()
                messagebox.showinfo(
                    "Exclusão concluída",
                    f"Foram excluídas {deleted} linha(s) com CNJ {cnj}."
                )
            except Exception as e:
                try:
                    conn.rollback()
                ...
                messagebox.showerror(
                    "Erro ao excluir",
                    f"Falha ao excluir registros do CNJ {cnj}:\n{e}"
                )
            finally:
                try:
                    cur.close()
        ...
        ttk.Button(
            frame_del,
            text="Excluir por CNJ",
            style="Primary.TButton",
            command=on_delete_cnj
        ).grid(row=0, column=2, padx=8, pady=4)

        frame_del.columnconfigure(1, weight=1)

        # ========================
        # 2) Exportação por cod_lote
        # ========================
        self.section_title(container, "📥 Exportar por cod_lote")

        frame_exp = ttk.Frame(container, style="Card.TFrame")
        frame_exp.pack(fill="x", pady=(0, 8))

        ttk.Label(
            frame_exp,
            text="Selecione um cod_lote para gerar um Excel apenas com esses processos.",
            style="Subtle.TLabel"
        ).grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 8))

        ttk.Label(frame_exp, text="cod_lote:", style="TLabel").grid(
            row=1, column=0, sticky="w", pady=4
        )

        cmb_cod_lote = ttk.Combobox(frame_exp, state="readonly", width=45)
        cmb_cod_lote.grid(row=1, column=1, padx=8, pady=4, sticky="w")

        # Carregar lista de cod_lote (DISTINCT) do banco
        def carregar_cod_lotes():
            conn, cur = conectar_ao_mysql()
            if not conn:
                return
            try:
                cur.execute("""
                    SELECT DISTINCT cod_lote
                    FROM encerramento
                ...
                cmb_cod_lote["values"] = valores
                if valores:
                    cmb_cod_lote.set(valores[0])
            except Exception as e:
                messagebox.showerror(
                    "Erro ao carregar cod_lote",
                    f"Falha ao buscar cod_lote distintos no banco:\n{e}"
                )
            finally:
                try:
                    cur.close()
        ...
        def on_export_lote():
            cod_lote_sel = cmb_cod_lote.get().strip()
            if not cod_lote_sel:
                messagebox.showwarning(
                    "Atenção",
                    "Selecione um cod_lote para exportar."
                )
                return

            path = filedialog.asksaveasfilename(
                title=f"Salvar planilha do cod_lote {cod_lote_sel}",
            ...
            def worker():
                ...
                try:
                    cur.execute(
                        "SELECT * FROM encerramento WHERE cod_lote = %s",
                        (cod_lote_sel,)
                    )
                    rows = cur.fetchall()
                    ...
                    self.after(0, lambda: messagebox.showerror(
                        ...
                        f"Falha ao buscar dados da tabela encerramento para o cod_lote {cod_lote_sel}:\n{e}"
                    ))
                    try:
                        cur.close()
                        conn.close()
                ...
                try:
                    ...
                    self.after(0, lambda: messagebox.showinfo(
                        ...
                        f"Arquivo gerado com {len(df)} linha(s) do cod_lote {cod_lote_sel}:\n{path}"
                    ))
                except Exception as e:
                    self.after(0, lambda: messagebox.showerror(
                        "Erro ao salvar Excel",
                        f"Falha ao salvar o arquivo Excel:\n{e}"
                    ))
        ...
        ttk.Button(
            frame_exp,
            text="Exportar para Excel (cod_lote selecionado)",
            style="Ghost.TButton",
            command=on_export_lote
        ).grid(row=1, column=2, padx=8, pady=4, sticky="w")

        frame_exp.columnconfigure(1, weight=1)

    def open_consulta_lote_window(self):
        """Janela para consultar processos por cod_lote e excluir 1+ CNJs selecionados."""
        win = tk.Toplevel(self)
        win.title("Consulta / Exclusão por cod_lote")
        ...
        # Filtro por cod_lote
        # ========================
        self.section_title(container, "🔎 Consulta de processos por cod_lote")

        filtro_frame = ttk.Frame(container, style="Card.TFrame")
        filtro_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(filtro_frame, text="cod_lote:", style="TLabel").grid(
            row=0, column=0, sticky="w", pady=4
        )

        cmb_cod_lote = ttk.Combobox(filtro_frame, state="readonly", width=50)
        cmb_cod_lote.grid(row=0, column=1, padx=8, pady=4, sticky="w")
        ...
        ttk.Button(
            filtro_frame,
            text="Carregar processos",
            style="Ghost.TButton",
            command=lambda: carregar_processos_por_lote()
        ).grid(row=0, column=2, padx=8, pady=4)
        ...
        # Lista (Treeview)
        # ========================
        lista_frame = ttk.Frame(container, style="Card.TFrame")
        lista_frame.pack(fill="both", expand=True)

        colunas = ("cnj", "cliente", "fase", "status", "data_exportacao")
        tree = ttk.Treeview(
            lista_frame,
        ...
        # Scrollbars
        scroll_y = ttk.Scrollbar(lista_frame, orient="vertical", command=tree.yview)
        scroll_y.pack(side="right", fill="y")
        tree.configure(yscrollcommand=scroll_y.set)

        # Cabeçalhos
        tree.heading("cnj", text="CNJ", anchor="w")
        tree.heading("cliente", text="Cliente", anchor="w")
        tree.heading("fase", text="Fase", anchor="w")
        tree.heading("status", text="Status", anchor="w")
        tree.heading("data_exportacao", text="Data Exportação", anchor="w")

        for col in colunas:
            tree.column(col, width=150, anchor="w", stretch=True)

        tree.tag_configure('oddrow', background=self.pal["row_odd"])
        tree.tag_configure('evenrow', background=self.pal["row_even"])

        # Mapa interno: iid -> cnj
        item_cnj_map: Dict[str, str] = {}

        def carregar_processos_por_lote():
            cod_lote_sel = cmb_cod_lote.get().strip()
            if not cod_lote_sel:
                messagebox.showwarning(
                    "Atenção",
                    "Selecione um cod_lote para carregar os processos."
                )
                return
            ...
            try:
                cur.execute("""
                    SELECT cnj, cliente, fase, status, data_exportacao
                    FROM encerramento
                    WHERE cod_lote = %s
                    ORDER BY cnj
                """, (cod_lote_sel,))
                ...
                messagebox.showerror(
                    "Erro na consulta",
                    f"Falha ao buscar processos do cod_lote {cod_lote_sel}:\n{e}"
                )
                try:
                    cur.close()
                    conn.close()
            ...
            for idx, row in enumerate(rows):
                cnj, cliente, fase, status, data_exportacao = row
                values = [
                    cnj or "",
                    cliente or "",
                    fase or "",
                    status or "",
                    (data_exportacao.strftime("%d/%m/%Y") if isinstance(data_exportacao, (date, datetime)) else str(data_exportacao) if data_exportacao else "")
                ]
                tag = 'evenrow' if idx % 2 == 0 else 'oddrow'
                iid = tree.insert("", "end", values=values, tags=(tag,))
                item_cnj_map[iid] = cnj
        ...
        def excluir_selecionados():
            ...
            if not cod_lote_sel:
                messagebox.showwarning(
                    "Atenção",
                    "Selecione um cod_lote antes de excluir."
                )
                return
            ...
            if not selecionados:
                messagebox.showwarning(
                    "Atenção",
                    "Selecione pelo menos um processo para excluir."
                )
                return

            cnjs = [item_cnj_map[iid] for iid in selecionados if item_cnj_map.get(iid)]

            if not cnjs:
                messagebox.showwarning(
                    "Atenção",
                    "Não foi possível identificar os CNJs selecionados."
                )
                return

            if not messagebox.askyesno(
                "Confirmar exclusão",
                f"Tem certeza que deseja excluir {len(cnjs)} processo(s) do cod_lote:\n{cod_lote_sel}?"
            ):
                return
            ...
            try:
                placeholders = ", ".join(["%s"] * len(cnjs))
                params = [cod_lote_sel] + cnjs
                sql = f"""
                    DELETE FROM encerramento
                    WHERE cod_lote = %s
                      AND cnj IN ({placeholders})
                """
                cur.execute(sql, params)
                deletados = cur.rowcount or 0
                conn.commit()
            except Exception as e:
                try:
                    conn.rollback()
                ...
                messagebox.showerror(
                    "Erro ao excluir",
                    f"Falha ao excluir registros selecionados:\n{e}"
                )
                try:
                    cur.close()
                    conn.close()
            ...
            for iid in selecionados:
                tree.delete(iid)
                item_cnj_map.pop(iid, None)

            messagebox.showinfo(
                "Exclusão concluída",
                f"Foram excluídas {deletados} linha(s) do cod_lote {cod_lote_sel}."
            )
